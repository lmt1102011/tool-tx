...
from core.config import GOLD, GOLD_DIM, DIM, TXT, NIGHT, GREEN, WARN
...
class BrowserScreen(MDScreen):
    ...
    def _build(self):
        ...
        col.add_widget(label("Trình duyệt tự động", style="H5", color=GOLD,
                             size=24, bold=True))
        col.add_widget(label("Agent chạy game giúp bạn — \"1 tab riêng\" trên "
                             "điện thoại hoặc Chrome máy tính.",
        ...
        guide.add_widget(label("HƯỚNG DẪN NHANH", style="Overline", color=GOLD_DIM,
                               size=12, bold=True))
        for line in [
            "1. Bấm START BROWSER trên điện thoại để mở Chromium Fork.",
            "2. Đăng nhập tài khoản trong tab vừa mở như web.",
            "3. Agent tự chọn cửa theo dữ liệu tool, server trừ đúng lượt.",
            "4. Trên máy tính dùng CHROME MÁY BẠN — kết nối CDP ngay tại máy.",
        ]:
